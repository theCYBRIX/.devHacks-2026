...
func _on_player_connected(alias : String, player_num : int) -> void:
	var entry := _create_player_entry(alias, player_num)
	
	if not is_node_ready():
		return
	
	player_list.add_child(entry, true, InternalMode.INTERNAL_MODE_FRONT)
	player_list.move_child(entry, player_num - 1)


func _on_player_disconnected(alias : String, player_num : int) -> void:
	if _list_updating:
		await list_updated
	
	if _player_items.has(alias):
		var list_entry := _player_items[alias]
		if list_entry and is_instance_valid(list_entry):
			list_entry.queue_free()
		_player_items.erase(alias)


func _refresh_player_list() -> void:
	_list_updating = true
	for player in PlayerManager.get_player_aliases():
		if not _player_items.has(player):
			_create_player_entry(player, PlayerManager.get_player_number(player))
...
func _create_player_entry(alias : String, player_num : int) -> PlayerEntry:
	var entry : PlayerEntry = PLAYER_ENTRY.instantiate()
	entry.name = str(player_num)
	entry.alias = alias
	entry.color = PlayerManager.get_player_color(alias)
	_player_items[alias] = entry
	return entry
